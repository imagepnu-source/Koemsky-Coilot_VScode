...
export function RadarGraph({ childProfile, categoryDevelopmentAges }: RadarGraphProps) {
  ...
  // 컴포넌트 내부 어딘가 상단
  const ANGLE_TICK_OFFSET_PX = 12;
  const renderAngleTick = ({ payload, x, y, cx, cy, textAnchor }: any) => {
    const angle = Math.atan2(y - cy, x - cx);
    const r = Math.hypot(x - cx, y - cy);
    const nx = cx + Math.cos(angle) * (r + ANGLE_TICK_OFFSET_PX);
    const ny = cy + Math.sin(angle) * (r + ANGLE_TICK_OFFSET_PX);
    return (
      <text x={nx} y={ny} textAnchor={textAnchor} fontSize={14} fill="#1f2937">
        {payload.value}
      </text>
    );
  };
  ...
  const data = useMemo(() => {
    ...
    return categories// List render — each item must have stable key
.map((category) => ({
      category: category === "수용 언어" ? "수용\n언어" : category,
      originalCategory: category,
      developmentAge:
        Math.round((categoryDevelopmentAges[category as PlayCategory] || 0) * 100) / 100,
      biologicalAge: Math.round(biologicalAge * 100) / 100,
    }))
  }, [categoryDevelopmentAges, biologicalAge])
  ...
  <div className="w-full min-w-0" ref={wrapRef}>
    {
      ...
      <h3 className="text-base font-bold text-center mb-4" style={{ fontSize: "16px" }}>
        발달 영역별 레이더 그래프
      </h3>

      <div className="w-full min-w-0 flex justify-center">
        {/* min-h로 컨테이너 자체 높이 보장 */}
        <div className="min-w-0 w-[95%] min-h-[450px]" ref={containerRef}>

          {/* 퍼센트 대신 '숫자 높이'로 고정 → -1 경고 차단 */}
          <ResponsiveContainer width="100%" height={450} minWidth={0} debounce={50}>
            <RadarChart data={data} /* outerRadius="85%" (옵션) */>
              <PolarGrid />
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
        {getGlobalKoreanNames().map((category) => (
          <div key={category} className="flex items-center">  {/* ← justify-between 제거 */}
            <span className="text-sm text-gray-700">{category}:</span>
            <span className="text-sm ml-[1ch]">
              {(categoryDevelopmentAges[category as PlayCategory] || 0).toFixed(2)}개월
            </span>
          </div>
        ))}
      </div>
