  ...
  return (
    ...
    >
      <div className="flex items-center justify-center gap-2">
        <h2 className="font-bold" style={{ fontSize: "14px" }}>{playNumber}. {playActivity.title}</h2>
        <Badge variant="outline" style={{ fontSize: "12px" }}>{playActivity.ageRange}개월</Badge>
      </div>

      <div className="flex items-center justify-between">
        <Button variant="ghost" size="sm" onClick={onBack} style={{ fontSize: "12px" }}><span className="font-bold">목록</span></Button>
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" style={{ fontSize: "12px" }} onClick={() => onNavigate?.(Math.max(1, playNumber - 1))} disabled={playNumber <= 1}>
            ...
            <span className="ml-1 font-bold">이전</span>
          </Button>
          <Button variant="ghost" size="sm" style={{ fontSize: "12px" }} onClick={() => onNavigate?.(playNumber + 1)} disabled={playNumber >= categoryTotalPlays}>
            <span className="mr-1 font-bold">다음</span>
      ...
      <div className="flex flex-col" style={{ gap: gapBetweenBoxes }}>
        ...
        {safetySection && (<SectionBox box={small} title={titleText} body={bodyText} label="안전 주의">{renderTextWithLineBreaks(safetySection.content)}</SectionBox>)}
