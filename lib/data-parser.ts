  ...
  sections.forEach((section, index) => {
    ...
    for (let i = 2; i < lines.length; i++) {
      ...
      if (parts.length >= 3) {
        ...
        // Parse age range (e.g., "0-4" -> min: 0, max: 4, or "12" -> min: 12, max: 12)
        ...
        if (ageParts.length === 1) {
          ...
          const singleAge = Number.parseInt(ageParts[0])
          minAge = singleAge
          maxAge = singleAge
        } else {
          // Range like "8-11" should be parsed normally
          minAge = Number.parseInt(ageParts[0])
          maxAge = Number.parseInt(ageParts[1])
        }
