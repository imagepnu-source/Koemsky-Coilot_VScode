...
import { saveTopAchievements } from "./storage-core"
import { saveCategoryRecord, loadCategoryRecord, createEmptyCategoryRecord } from "./storage-category"
import { calculateAchieveMonthOfThePlay } from "./category-achievements-manager"
import { calculateCategoryDevelopmentalAgeFromTopAchievements } from "./development-calculator"
...
export function generateTestData(
  ...
  categories.forEach((category, categoryIndex) => {
    ...
    const actualCount = categoryActivities.length
    const targetCount = Math.min(testDataCount, actualCount)

    console.log(`[v0] Processing category ${category} with ${actualCount} activities`)
    ...
    for (let i = 0; i < targetCount; i++) {
      ...
      do {
        randomIndex = Math.floor(Math.random() * categoryActivities.length)
      } while (usedIndices.has(randomIndex))

      usedIndices.add(randomIndex)
      const randomActivity = categoryActivities[randomIndex]

      const rotatingLevel = (i % 5) + 1

      const developmentAge = calculateAchieveMonthOfThePlay(randomActivity.minAge, randomActivity.maxAge, rotatingLevel)
...
.map((activity) => ({
    ...
    categoryAchievements.forEach((achievement) => {
      categoryRecord.graphData.push({
        achieveDate: achievement.achievedDate,
        playNumber: achievement.playNumber,
        achievedLevel_Highest: achievement.achievedLevel,
        AchieveMonthOfThePlay: achievement.developmentAge,
        achievedMonth: achievement.developmentAge,
      })
    })

    categoryRecord.topAchievements = categoryAchievements// List render — each item must have stable key
.map((achievement) => ({
      playNumber: achievement.playNumber,
      playTitle: achievement.playTitle,
      achievedLevel: achievement.achievedLevel,
      developmentAge: achievement.developmentAge,
      achievedDate: achievement.achievedDate,
      category: achievement.category,
    }))

    categoryRecord.categoryDevelopmentalAge = calculateCategoryDevelopmentalAgeFromTopAchievements(
      categoryRecord.topAchievements,
    )

    saveCategoryRecord(categoryRecord)
    console.log(
      `[v0] Saved CategoryRecord for ${category} with ${categoryRecord.playData.length} PlayData, ${categoryRecord.graphData.length} GraphData, and ${categoryRecord.topAchievements.length} TopAchievements entries`,
    )
  ...
  saveTopAchievements(testAchievements)

  downloadCSV(csvData.join("\n"), `test_data_${new Date().toISOString().split("T")[0]}.csv`)

  console.log(`[v0] Saved global top achievements for ${Object.keys(testAchievements).length} categories`)
